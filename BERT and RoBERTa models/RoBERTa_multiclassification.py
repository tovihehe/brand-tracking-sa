...
class RobertaSentiment(torch.nn.Module):
    def __init__(self, n_classes):
        super(RobertaSentiment, self).__init__()
        self.robs = RobertaModel.from_pretrained("roberta-base")
        self.dropout = torch.nn.Dropout(DROPOUT_PROB)
        self.classifier = torch.nn.Linear(768, n_classes)

    def forward(self, input_ids, attention_mask):
        out1 = self.robs(input_ids=input_ids, attention_mask=attention_mask)
        hidden_state = out1[0]
        pooler = hidden_state[:, 0]
        pooler = torch.nn.ReLU()(pooler)
        pooler = self.dropout(pooler)
        out2 = self.classifier(pooler)
        return out2


### Create an instance of the model
model = RobertaSentiment(NUM_CLASSES)
model = model.to(device)

### Define model and optimizer and scheduler
...
ax.set_ylim([0, 1])
fig.savefig("ROBERTA_train_VS_val.png");
...
print("ROBERTA Test accuracy", test_acc.item())
...
print("ROC CURVE \n")
#print(multiclass_roc_curve(y_test.numpy(), y_pred_probs.numpy(), NUM_CLASSES));

print("PR CURVE \n")
#print(multiclass_precision_recall_curve(y_test.numpy(), y_pred_probs.numpy(), NUM_CLASSES));
